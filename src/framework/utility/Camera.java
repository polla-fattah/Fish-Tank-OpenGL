...
public class Camera implements Input{
	 ...
	 * State of key press.
	 */
	private int wKey = 0, aKey = 0, sKey = 0, dKey = 0, iKey = 0, jKey = 0;
	/**
...
////// Directional Axes Methods /////////////////////////////////////
	 ...
	 */
	public void reset(){
		eye.set(180.f,0.f,170.f);									// Initially situate at origin
		view.set(-0.5f,0.f,-1.f);									// Set view direction forward
		forward.set(-0.5f,0.f,-1.f);								// Forward axis follows view direction
		right.set(400.f,0.f,0.f);									// Right axis initially right along world coordinates
...
/////// Input handling functions ////////////////////////////////////
	...
	public void handleKey(char key, int state, int mX, int mY) {
		switch(key){
			...
			case 'I':
			case 'i':
				iKey = state;			// 'i' key is pressed up (0) or down (1)
				break;
			case 'J':
			case 'j':
				jKey = state;			// 'j' key is pressed up (0) or down (1)
				break;
			case ' ':					// spacebar
